...
const CreateAuctionPage = () => {
	...
	const handleSubmit = async (event) => {
		...
		if (!itemName && !itemDescription && !startingPrice && !auctionEndDate && !category) {		
			...
			const epochEndTime = parseInt(auctionEndDate); // Convert string to integer (milliseconds)
			...
			// If auction type is Dutch, ensure Buy Now Price is greater than Starting Price
				...
				auctionData.append("auctionEndDate", epochEndTime); // Send as integer (milliseconds)
				...
				<form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
						...
						<input
							type="datetime-local"
							id="auctionEndDate"
							value={auctionEndDate ? new Date(parseInt(auctionEndDate)).toISOString().slice(0, 16) : ""}
							onChange={(e) => {
								const date = new Date(e.target.value);
								const epochTime = date.getTime(); // Convert to epoch time in milliseconds (integer)
								setAuctionEndDate(epochTime.toString()); // Store as string for state
							}}
							required
							style={{
								width: "100%",
